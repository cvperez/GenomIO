...
Test whether a DNA encoder can also represent CDS metadata, and compare it with a separate text encoder.

### Experimental Setup

The five-record sample uses DNABERT-S for DNA, DNABERT-S for metadata as a control, and `sentence-transformers/all-MiniLM-L6-v2` for metadata. Metadata is extracted from FASTA headers. The script computes within-modality cosine matrices, silhouette scores, and DNA-to-metadata similarities in the DNABERT-S space.
...
```bash
python3 embedder_benchmark/experiment_3_dual_modality/experiment_3_dual_modality.py
```
...
`experiment_3_results.csv` contains embedding metrics. Four heatmaps show DNA, the two metadata representations, and the DNA/metadata cross-modal comparison.

### Main Findings

The report records high cross-record similarities for DNABERT-S metadata and inconsistent cross-modal ranks. MiniLM produces a different metadata similarity pattern. The sample is small; Experiment 4 provides the larger comparison. These observations do not establish the tokenizer mechanism proposed in the historical report.
...
**Script**: `embedder_benchmark/experiment_3_dual_modality/experiment_3_dual_modality.py`
