...
Evaluate the larger retrieval routes using FAISS and compare the recorded metrics with the Experiment 6 baseline.

### Experimental Setup

The reported sample has 50 CDS records per accession group, 20 groups, and the 300–900 bp filter. DNA uses DNABERT-S with HNSW (`M=32`, `efConstruction=200`, `efSearch=64`); MiniLM metadata uses Flat inner-product search. Evaluation excludes self matches and uses same-group relevance. Metrics are P@1/5/10/20, Recall@10/20, MRR, and index statistics. `EXP6_RESULTS` is a hardcoded historical comparison, not a file dependency.
...
```bash
python3 embedder_benchmark/experiment_10_faiss_retrieval_scale/experiment_10_faiss_retrieval_scale.py
```
...
`experiment_10_results.csv` and `experiment_10_summary.csv` contain retrieval metrics; `experiment_10_faiss_stats.csv` contains index statistics. DNA/text indices, per-species plots, and a comparison with Experiment 6 are generated.

### Main Findings

The report gives DNA-to-DNA P@1 of 0.655 and text-to-DNA P@1 of 0.906. Its text comparison with Experiment 6 also includes a metadata-name correction, so the improvement must not be attributed to FAISS alone.
...
**Script**: `embedder_benchmark/experiment_10_faiss_retrieval_scale/experiment_10_faiss_retrieval_scale.py`
