...
Measure the same record-ID retrieval routes on a larger corpus using brute-force cosine ranking.

### Experimental Setup

The script selects up to 50 CDS records per corpus file, filtered to 300–900 bp; the report uses 1,000 records. It embeds DNA with DNABERT-S and metadata with MiniLM. Each query excludes itself, leaving 49 same-group relevant records in the reported setup. Metrics include P@1/5/10/20, Recall@10/20, and MRR.
...
```bash
python3 embedder_benchmark/experiment_6_retrieval_scale/experiment_6_retrieval_scale.py
```
...
`experiment_6_results.csv` contains per-query metrics; `experiment_6_summary.csv` aggregates by route. Plots show P@1/P@10 by species and a metric summary.

### Main Findings

The historical report gives DNA-to-DNA P@1 of 0.655 and text-to-DNA P@1 of 0.601. Experiment 10 reports different text results after metadata-name handling changed. The current script reads corrected organism names, so this old text score is not a guaranteed reproduction target.
...
**Script**: `embedder_benchmark/experiment_6_retrieval_scale/experiment_6_retrieval_scale.py`
