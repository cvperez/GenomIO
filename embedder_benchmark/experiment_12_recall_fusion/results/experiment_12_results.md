...
Compare DNA-only, metadata-only, and reciprocal-rank-fused retrieval; test how reducing query metadata changes that comparison.

### Experimental Setup

The reported setup contains 1,000 CDS records (50 per group, 300–900 bp). Index A is DNABERT-S/HNSW (`M=32`, `efConstruction=200`, `efSearch=64`); Index B is CLS-pooled SapBERT/Flat. Same-group relevance excludes the query. K is 1,3,5,10,15,20,30,40,49. Reciprocal rank fusion uses `RRF_C=60` and text weights 1 and 2. The follow-up ablation uses organism-only text queries and compares SapBERT with MiniLM. No sequence is generated.
...
```bash
python3 embedder_benchmark/experiment_12_recall_fusion/experiment_12_recall_fusion.py
```
...
After the full run has produced `results/experiment_12_index_dna.faiss`, `results/experiment_12_index_text.faiss`, and `results/experiment_12_summary.csv`, run:

```bash
python3 embedder_benchmark/experiment_12_recall_fusion/experiment_12_recall_fusion.py ablation
```

Those prerequisites are not included in this checkout. The ablation reuses both saved indices and the full-run summary, but still embeds its text queries and builds the MiniLM comparison. `SELECTED_ENCODER` is configured in this script; it does not automatically read Experiment 11's output.
...
`experiment_12_results.csv` and `experiment_12_summary.csv` contain full-run query and aggregate metrics. The run saves DNA/text `.faiss` indices, plots, and `experiment_12_run.log`. The ablation writes `experiment_12_ablation_summary.csv`, `experiment_12_encoder_compare.csv`, ablation plots, and its separate run log.

### Main Findings

With full metadata, the report gives SapBERT P@1 of 0.974 and text-weighted fusion P@1 of 0.996; both have Recall@49 of 0.893. With organism-only queries, it reports P@1 of 0.800 for text and 0.907 for fusion, with Recall@49 of 0.739 for both. These comparisons assume a known organism represented in the corpus and do not establish a gain in reconstructed-sequence accuracy.
...
**Script**: `embedder_benchmark/experiment_12_recall_fusion/experiment_12_recall_fusion.py`
...
context. Neither appears in text's list at all (B-rank "not ranked"), so after fusion they fall out
...
### Reproduce
```bash
python embedder_benchmark/experiment_12_recall_fusion/experiment_12_recall_fusion.py            # §1–§12 (full experiment)
python embedder_benchmark/experiment_12_recall_fusion/experiment_12_recall_fusion.py ablation   # §13 organism-only ablation
```
