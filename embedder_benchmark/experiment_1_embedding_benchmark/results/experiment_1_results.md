...
Compare the embedding geometry of the model registry on a small, fixed DNA sample before the larger evaluation in Experiment 2.

### Experimental Setup

The script selects five 300–900 bp CDS records from four accession files using `SEQUENCE_SOURCES`. Labels are `[0, 0, 1, 2, 3]`, so only one same-organism pair exists. `MODEL_REGISTRY` contains 16 entries. It measures cosine similarity, inter-group distance, silhouette, and inference time. The GERM entry falls back to DNABERT-2 if no local checkpoint is found; it is not an independent model result in that case.
...
```bash
python3 embedder_benchmark/experiment_1_embedding_benchmark/experiment_1_embedding_benchmark.py
```
...
```bash
python3 embedder_benchmark/experiment_1_embedding_benchmark/experiment_1_embedding_benchmark.py --only DNABERT_S
```
...
`experiment_1_results.csv` contains model metrics; `experiment_1_<MODEL>_heatmap.png` contains pairwise cosine similarities.

### Main Findings

The stored CPU table has 13 entries. DNABERT_1 has the highest reported silhouette (0.2686); DNABERT_S reports 0.1794. This five-record comparison is too small to establish a general model ranking. The larger comparison belongs to Experiment 2.
...
**Script**: `embedder_benchmark/experiment_1_embedding_benchmark/experiment_1_embedding_benchmark.py`
...
One heatmap per model in `embedder_benchmark\experiment_1_embedding_benchmark/results/experiment_1_{MODEL}_heatmap.png` (13 files).
