...
Measure how index latency, build cost, size, and agreement change with corpus size.

### Experimental Setup

The code selects up to 150 CDS records per corpus file, filtered to 300–900 bp, and benchmarks prefixes of 100, 250, 500, 1,000, 2,000, and 3,000 vectors. These prefixes follow file order; they are not separately balanced samples. It compares normalized DNABERT-S and MiniLM vectors with Flat and HNSW search (`M=32`, `efSearch=64`), using 200 latency repetitions and top-1 agreement with brute-force cosine.
...
```bash
python3 embedder_benchmark/experiment_8_faiss_scale/experiment_8_faiss_scale.py
```
...
`experiment_8_results.csv` contains modality/index/size measurements; `experiment_8_latency_vs_n.png` and `experiment_8_accuracy.png` summarize latency and agreement.

### Main Findings

The stored DNA timings are nearly equal at 1,000 vectors (52.70 µs Flat, 51.52 µs HNSW), with HNSW faster at 2,000 and 3,000. Text timings favor Flat at 1,000 and HNSW at 2,000 and 3,000. These hardware-specific measurements do not justify extrapolating to untested million-record indices.
...
**Script**: `embedder_benchmark/experiment_8_faiss_scale/experiment_8_faiss_scale.py`
